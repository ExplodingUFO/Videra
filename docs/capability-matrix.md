...
| Core model import | Yes | Current runtime supports `.gltf`, `.glb`, and `.obj` loading on the viewer path |
...
| `Import` | Asset ingestion for viewer/runtime scenes | Currently implemented inside `Videra.Core`; extraction to dedicated packages is planned in `v1.20` | Not yet a dedicated package |
