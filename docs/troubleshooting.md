...
- `Try next: Waterfall proof` is the third demo path on the same Avalonia shell, and `Try next: Scatter proof` covers the repo-owned scatter path on that same shell.
...
- use `Try next: Waterfall proof` when the issue is about the second chart control instead of the cache-backed path, and use `Try next: Scatter proof` when the issue is about the direct scatter control path; the shipped control line also includes `ScatterChartView`
