...
- Confirm pull-request `sample-contract-evidence` stayed green for `Videra.SurfaceCharts.Demo`, including `Start here: In-memory first chart`, `Explore next: Cache-backed streaming`, `Try next: Waterfall proof`, the `Copy support summary` workflow, and the SurfaceCharts runtime evidence step.
