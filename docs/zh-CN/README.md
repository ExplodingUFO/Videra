...
当前公开控制层包括 `SurfaceChartView`、`WaterfallChartView` 和 `ScatterChartView` 三个 Avalonia 控件；`Videra.SurfaceCharts.Processing` 只在 surface/cache-backed 路径需要，不是每条 chart 路径都必须安装。独立 Demo 现在通过 repo-owned `Try next: Scatter proof` 也覆盖 scatter 路径。
...
- [独立 Demo](../../samples/Videra.SurfaceCharts.Demo/README.md)：`Videra.SurfaceCharts.Demo`，作为 repository-only 的 support-ready chart 参考应用，保留 `Copy support summary` 复现路径，展示 source 切换、built-in interaction、probe workflow、`Try next: Scatter proof` 与 rendering path truth
