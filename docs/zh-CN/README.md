...
当前公开控制层包括 `VideraChartView` 以及 `Plot.Add.Surface`、`Plot.Add.Waterfall`、`Plot.Add.Scatter` 三个 Avalonia 控件；`Videra.SurfaceCharts.Processing` 只在 surface/cache-backed 路径需要，不是每条 chart 路径都必须安装。独立 Demo 现在通过 repo-owned `Try next: Analytics proof` 和 `Try next: Scatter proof` 也覆盖分析与 scatter 路径。
