...
独立 Demo 会先把 `Start here: In-memory first chart` 作为默认入口，等基线 first chart 跑通后，再切到 `Explore next: Cache-backed streaming`，需要第二个控件证明时再看 `Try next: Waterfall proof`，需要 scatter 路径证明时再看 repo-owned `Try next: Scatter proof`。
