...
`Videra.SurfaceCharts.Avalonia` 提供专用的 `VideraChartView` 控件；`Plot.Add.Surface`、`Plot.Add.Waterfall`、`Plot.Add.Scatter` 是这个控件上的 chart authoring 与 runtime data-loading 入口。它独立于 `VideraView`，不承载 viewer 模式，也不耦合到 `VideraView` 的选择、标注或相机链路。
