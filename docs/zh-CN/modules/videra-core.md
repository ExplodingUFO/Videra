...
dotnet add package Videra.Core --version 0.1.0-alpha.4 --source github-ExplodingUFO
```
...
- `SceneDocument` 保留 imported asset 的 backend-neutral 真相，backend 恢复会从这份 scene truth 重建资源，而不是长期依赖 software staging path。
