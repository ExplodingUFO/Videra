...
- inspection 问题需要可重放上下文时：同时附上 `VideraDiagnosticsSnapshotFormatter` 输出和 `VideraInspectionBundleService.ExportAsync(...)` 导出的 bundle；`CanReplayScene` 和 `ReplayLimitation` 描述的是 replayability 语义，导出 bundle 时也要一起带上
