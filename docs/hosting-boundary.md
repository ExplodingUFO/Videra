...
`Videra.SurfaceCharts.*` is the shipped public chart package family. It is not part of the `VideraView` hosting/runtime path, and `Videra.SurfaceCharts.Processing` is only needed for the surface/cache-backed chart path.
...
| `Charts` | Shipped public chart package family | `VideraChartView` with `Plot.Add.Surface`, `Plot.Add.Waterfall`, and `Plot.Add.Scatter`, and `Videra.SurfaceCharts.*` |
