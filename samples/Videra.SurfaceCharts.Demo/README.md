...
- a copyable evidence-only support summary that includes active chart control type, runtime/assembly identity, backend/display environment variables, last cache-load failure for the cache-backed path (no source switch on failure), selected scenario id/name/update mode, point counts, FIFO capacity, pickability, rendering status, and dataset/source details; cache-backed source unavailable and there is no scenario/data-path fallback. The previous chart source remains active on load failure; these values are support evidence, not stable benchmark guarantees
