...
- The current alpha-ready `green` line is repository verification + native validation + packaged consumer smoke + sample-contract evidence, with `quality-gate-evidence` keeping the packaged consumer path, curated Core test surfaces, and `Videra.SurfaceCharts.Demo` warning-clean while `run-benchmarks` remains the explicit label-gated benchmark-review switch rather than a hard numeric blocker
