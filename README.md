...
- `Videra.SurfaceCharts.*` now ships as a public `alpha` package line, `VideraChartView` is the current Avalonia chart control, `Plot.Add.Surface`, `Plot.Add.Waterfall`, and `Plot.Add.Scatter` are its chart authoring entrypoints, `Videra.SurfaceCharts.Processing` is needed for the surface/cache-backed path, and `Videra.SurfaceCharts.Demo` remains repository-only
...
| `Videra.MinimalAuthoringSample` | Core-only static-scene authoring reference for primitives, axis triads, scale bars, and repeated marker instances without model files |
| `Videra.AvaloniaWorkbenchSample` | Optional sample-first workbench workflow for authored scenes, model loading, structured support capture, and chart output/palette evidence |
...
`Videra.MinimalAuthoringSample` is the quickest Core-only static-scene authoring proof. It exercises `SceneAuthoring.Create(...)`, built-in primitives, `AddAxisTriad(...)`, `AddScaleBar(...)`, and `AddInstances(...)` while keeping the output as retained `SceneDocument` / `InstanceBatchEntry` truth instead of adding an ECS, update loop, material graph, backend feature, runtime gizmo framework, or hidden fallback layer.
`Videra.AvaloniaWorkbenchSample` remains an optional sample-first adoption workflow. It composes public APIs only and copies structured evidence for authored scene shape, selected marker identity, SurfaceCharts output evidence, palette settings, and diagnostics snapshot status without becoming a default package dependency or per-frame diagnostics UI.
